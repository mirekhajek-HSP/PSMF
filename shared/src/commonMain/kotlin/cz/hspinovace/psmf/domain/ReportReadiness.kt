...
sealed interface ReportProblem {
    ...
    data class CardWithoutReason(val minute: Minute) : ReportProblem

    data object MissingCommentary : ReportProblem

    data class AssessmentIncomplete(val side: TeamSide) : ReportProblem

    data object MissingResult : ReportProblem

    data class ScoreDisagreesWithGoals(val recorded: Score, val fromGoals: Score) : ReportProblem

    data object MissingOfficials : ReportProblem

    data class MissingLineup(val side: TeamSide) : ReportProblem

    data class MissingConfirmation(val party: ConfirmingParty) : ReportProblem
}
 ...
 */
fun Match.reportProblems(): List<ReportProblem> = buildList {
